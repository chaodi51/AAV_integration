...
with open(IS_reads, 'r') as IS_reads:
    ...
    for line in IS_reads:
        ...
        # print(f"{paired_read_name}")

# exclude_flags = 0x4 | 0x100 | 0x800 # not primary reads, only find reads in primary R2
...
for read in bam_file:
    ...
    # soft_clip_cigar = cigar.replace("H", "S") if "H" in cigar else cigar
    ...
    # sometimes, IS and sonication can be both on R1/R2, but the position can only be obtained from where the paired read start

    # sonication reads can also be supplementary alignments as IS reads host part can be supplementary alignments
    if (chrom in chromosomes) and read_name in IS_paired_read_dict:
        # if not chrom in chromosomes:
        # print(f"{read_name},{flag},{chrom},{cigar}")
        soni_read_uniqueness = 'host_multi' if read.has_tag("XA") else 'host_uniq'
            
        # M_pos = cigar_match_position(soft_clip_cigar)
        ...
        # print(f"{cigar},{strand},{site}")
        if site != 'not_valid':
            if lib_method == 'slimPCR' and (flag & 0x80):
                sonication_read_slimPCR = ";".join([read_name, soni_read_uniqueness, chrom, str(site)])
                ...
                sonication_read_TES = ";".join([read_name, soni_read_uniqueness, chrom, str(site)])
...
data = []
IS_info = 'IS_reads[read;host_uniquess;vector_chrom;vector_align_clip_pos;vector_strand;vector_cigar;'\
...
for event_id, event_info_list in IS_event_dict.items():
    ...
    for each in event_info_list:
        ...
        all_IS_reads_uniqueness.append(each.split(';')[1])
    ...
    # add label if IS has one unique origin or possible multiple origins        
    IS_origin = 'fixed' if 'host_uniq' in all_IS_reads_uniqueness else 'uncertain'
        
    # get unique number of soniciate sites, different reads can have the same site
    for soni_read in IS_sonication_reads:
        _, soni_uniqueness, soni_chr, soni_pos = soni_read.split(';')
        soni_site = soni_chr + ';' + soni_pos
        IS_sonication_sites.add(soni_site)

    nChops = len(IS_sonication_sites)

    if nReads >=1 and nChops >= 1: # keep IS events with >=2 sonication sites

        # print(f"{event_id},{nReads},{nChops}")
        count += 1
        event_lab = 'IS_' + str(count)
        data.append([event_lab, IS_origin, chrom, IS_position, nReads, nChops, event_info_list, list(IS_sonication_reads)])

df = pd.DataFrame(data, columns = ["event_lab", "IS_origin", "chrom", "IS_pos", "nReads", "unique_sonications", IS_info, "sonication_sites"])
