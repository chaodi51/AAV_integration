---
title: "Plots for integration sites (IS) - human"
output:
  html_document:
    toc: true
    toc_float: true
...
Number of random IS is the same as the real number of IS to be compared.  
```{r}
...
# IS_tab <- read.csv('/mnt/bfx-ops/projects/2023/aav_integration/v1.0.0/data/interim/IS_results_final/IS_events_anno/shortRead/240405_M05240_0149_000000000-L9FLR__IS_final.anno.csv', check.names = FALSE, header = TRUE)
# map_tab <- read.csv("/mnt/bfx-ops/projects/2023/aav_integration/v1.0.0/data/interim/shortRead/240405_M05240_0149_000000000-L9FLR__mapped_table.tsv", sep = '\t', header = TRUE)
...
# kable(head(cgc_98))
IS_tab <- IS_tab %>%
    mutate(host_closest_gene = map_chr(str_split(host_closest_TSS_info, ";"), 1))

IS_tab$CancerGene <- ifelse(IS_tab$host_closest_gene %in% cancGenes, TRUE, FALSE)
```

**Frequencies of IS NEAR(upstream 100kb of TSS) or IN cancer genes**
```{r}
data <- IS_tab %>% filter(CancerGene == TRUE & (host_inGene == 1 | (host_closest_TSS_dist > 0 & host_closest_TSS_dist < 100000))) %>% 
    select(sample) %>%
    group_by(sample) %>%
    summarise(count = n())
...
data <- IS_tab %>% filter(CancerGene == TRUE & host_inGene == 1) %>%
    select(sample) %>% 
    group_by(sample) %>% 
    summarise(count = n())
...
CancGeneTable <- IS_tab %>% 
    filter(CancerGene == TRUE & (host_inGene == 1 | (host_closest_TSS_dist > 0 & host_closest_TSS_dist < 100000))) %>%
    select(sample, chrom, IS_pos, host_closest_gene, host_inGene, host_closest_TSS_info, host_closest_TSS_dist)
...
output_file <- paste0('human_random', '_IS_associated_cancGeneTable')
...
host.band <- read.chromInfo("/mnt/bfx-ops/references/Homo_sapiens/UCSC/hg38/Annotation/chromInfo.txt.gz")$df %>% 
...
xrange = c(cytoband %>% filter(chr %in% c(seq(1:22),"X", "Y"))%>% pull(end),
            cytoband %>% filter(!chr %in% c(seq(1:22),"X", "Y")) %>% mutate(length=end-start) %>% pull(length))
normal_chr_index = 1:24
zoomed_chr_index = 25:27
...
data <- IS_tab %>%
    mutate(host_closest_gene = map_chr(str_split(host_closest_TSS_info, ";"), 1)) %>%
    select(sample, chrom, IS_pos, nReads, unique_sonications, host_closest_gene) %>%
    group_by(sample) %>%
...
for (sample in samples) {
    p <- ggplot(df[df$sample == sample, ], aes(x=sample, y = Percentage, fill = host_closest_gene)) +
        geom_bar(stat = "identity") +
