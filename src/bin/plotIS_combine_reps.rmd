---
title: "Plots for integration sites (IS) -- combine replicates"
output:
  html_document:
    toc: true
    toc_float: true
...
master_mapping <- read.csv(params$master_mapping, sep = '\t')
...
# IS_tab <- read.csv("/home/ubuntu/aav_integration/v1.0.0/data/interim/IS_results_final/IS_events_anno/shortRead/240112_VH00163_77_AAF5V37M5__IS_final.anno.csv", check.names = FALSE, header = TRUE)
...
## Combine with meta data table
```{r}
# master_mapping <- read.csv("/home/ubuntu/aav_integration/v1.0.0/configs/master_mapping_anno.tsv", sep = "\t")
master_mapping <- master_mapping %>% mutate(sample_name = paste(condition, bio_replicate, sep = "_"))

master_mapping_1 <- master_mapping[, -c(3, 4)] |> mutate(Sample = gsub("_a1", "", Sample))
IS_tab <- left_join(IS_tab, master_mapping_1, by = c("sample" = "Sample"))
master_mapping_2 <- master_mapping[, -c(1, 3)] |> mutate(Sample = gsub("_a1", "", Sample))
map_tab <- left_join(map_tab, master_mapping_2, by = c("sample" = "Sample"))

IS_tab$sample_name <- factor(IS_tab$sample_name, levels = c("low_dose_rep1", "low_dose_rep2",
                                            "low_dose_with_standard_rep1", "low_dose_with_standard_rep2",
                                            "high_dose_rep1", "high_dose_rep2",
                                            "high_dose_with_standard_rep1", "high_dose_with_standard_rep2"))
map_tab$sample_name <- factor(map_tab$sample_name, levels = c("low_dose_rep1", "low_dose_rep2",
                                            "low_dose_with_standard_rep1", "low_dose_with_standard_rep2",
                                            "high_dose_rep1", "high_dose_rep2",
                                            "high_dose_with_standard_rep1", "high_dose_with_standard_rep2","mock_control_rep1","mock_control_rep2","standard_control_rep1","standard_control_rep2"))
```

# Fig 6: Number of integration sites
**raw number**  
```{r}
data <- IS_tab %>% group_by(sample_name, tech_replicate) %>% summarise(count = n(), .groups = 'drop')
tot_reads <- map_tab %>%
    group_by(sample_name, tech_replicate) %>%
    summarise(tot_input = sum(tot_input), .groups = 'drop')

data1 <- left_join(tot_reads, data, by = c("sample_name", "tech_replicate"))
data1 <- data1 %>%
  mutate(norm_count = count / tot_input * 10^6)

summary_data <- data1 %>%
    group_by(sample_name) %>%
    summarise(mean_count = mean(count),
                se_count = sd(count) / sqrt(n()), .groups = 'drop') %>% drop_na()


ggplot(summary_data, aes(x = sample_name, y = mean_count)) + 
...
summary_data <- data1 %>%
    ...
    summarise(mean_count = mean(norm_count),
                se_count = sd(norm_count) / sqrt(n()), .groups = 'drop') %>% drop_na()
...
data_summary <- IS_tab %>% group_by(sample, sample_name, tech_replicate) %>% 
    summarise(Proportion = mean(host_inGene == 1), .groups = "drop") %>%
    group_by(sample_name) %>%
    summarise(mean_proportion = mean(Proportion), 
                se_proportion = sd(Proportion) / sqrt(n()), .groups = 'drop') %>% drop_na()

ggplot(data_summary, aes(x = sample_name, y = mean_proportion)) +
    geom_bar(stat = "identity") +
    geom_errorbar(aes(ymin = mean_proportion - se_proportion, ymax = mean_proportion + se_proportion), width = 0.2) +
...
```{r}
data_summary <- IS_tab %>% group_by(sample, sample_name, tech_replicate) %>% 
    summarise(Proportion = mean(cpg_ingene == 1), .groups = "drop") %>%
    group_by(sample_name) %>%
    summarise(mean_proportion = mean(Proportion), 
                se_proportion = sd(Proportion) / sqrt(n()), .groups = 'drop') %>% drop_na()
...
```{r}
data_summary <- IS_tab %>% group_by(sample, sample_name, tech_replicate) %>% 
    summarise(Proportion = mean(enh_ingene == 1), .groups = "drop") %>%
    group_by(sample_name) %>%
    summarise(mean_proportion = mean(Proportion), 
                se_proportion = sd(Proportion) / sqrt(n()), .groups = 'drop') %>% drop_na()
...
df_grouped <- data %>%
    group_by(sample, sample_name, tech_replicate, distance_bin) %>%
    summarise(frequency = n(), .groups = 'drop') %>%
    group_by(sample_name, distance_bin) %>%
    summarise(mean_frequency = mean(frequency), 
                se_frequency = sd(frequency) / sqrt(n()), .groups = 'drop') %>% drop_na()

ggplot(df_grouped, aes(x = distance_bin, y = mean_frequency, fill = sample_name)) +
    geom_bar(stat = "identity", position=position_dodge(width = 0.9), width = 0.7) +
    geom_errorbar(aes(ymin = mean_frequency - se_frequency, ymax = mean_frequency + se_frequency), position=position_dodge(width = 0.9), width = 0.2) +
    theme_minimal() +
...
ggplot(data, aes(x = sample_name, y = mean_count)) + 
    geom_bar(stat = "identity") +
    geom_errorbar(aes(ymin = mean_count - se_count, ymax = mean_count + se_count), width = 0.2) +
    theme_minimal() +
    labs(x = "Sample", y = "IS number", title = "# of unique IS") +
...
data <- IS_tab %>%
    select(sample_name, nReads)

ggplot(data, aes(y = nReads)) +
    geom_boxplot() +
    theme_minimal() +
    facet_wrap(~sample_name, scales = 'free', ncol = 8) + 
    labs(x = "Sample", y = "# reads per IS", title = "Read abundance per IS") +
    theme(axis.text.x = element_blank(), 
        strip.text.x = element_text(angle = 45, hjust = 0, vjust = 0, size = 10), 
        plot.title = element_text(hjust = 0.5, size = 20),
...
```{r,fig.width= 8, fig.height= 7}
data <- IS_tab %>%
    select(sample_name, unique_sonications)
...
data <- IS_tab %>%
    select(sample, sample_name, unique_sonications)
...
freq_df <- data %>%
    group_by(sample, sample_name, unique_sonications) %>%
    summarise(Count = n()) %>%
    mutate(Percentage = (Count / sum(Count)) * 100) %>%
    group_by(sample_name, unique_sonications) %>%
    summarise(mean_Percentage = mean(Percentage),
                se_Percentage = sd(Percentage) / sqrt(n()), .groups = 'drop') %>% drop_na()

ggplot(freq_df, aes(x = unique_sonications, y = mean_Percentage)) +
    geom_bar(stat = "identity") +
    geom_errorbar(aes(ymin = mean_Percentage - se_Percentage, ymax = mean_Percentage + se_Percentage), width = 0.2) +
    theme_minimal() +
...
ggplot(data, aes(x = sample_name, fill = repeat_class)) +
...
data <- IS_tab %>%
    select(sample_name, vector_align_clip_pos, nReads)
data$region <- ifelse(between(data$vector_align_clip_pos, ITR_5P_e + 1, ITR_3P_s - 1), "others", "ITR")

data <- data[order(data$sample_name, data$vector_align_clip_pos),]
...
ggplot(data, aes(x = vector_align_clip_pos, fill = region, color = region)) +
    ...
    facet_wrap(~sample_name, scales = 'free', ncol = 4, strip.position = "top") + 
...
data <- IS_tab %>%
    mutate(orth_closest_gene = map_chr(str_split(orth_closest_TSS_info, ";"), 1)) %>%
    select(sample_name, chrom, IS_pos, nReads, unique_sonications, orth_closest_gene) %>%
    group_by(sample_name) %>%
...
data1 <- data %>%
    ...
    ungroup() %>%
    group_by(sample_name, Top10_or_Rest) %>%
    summarise(Sum_Percentage = sum(Percentage), ..groups = 'drop') %>%
    ungroup()

ggplot(data1, aes(x = sample_name, y = Sum_Percentage, fill = Top10_or_Rest)) +
    geom_bar(stat = "identity") +
    labs(y = "Percentage", x = "Sample", fill = "Category") +
    ggtitle("Top 10 vs Rest Percentages") + 
    scale_fill_manual(values = c('Top 10' = '#ff00008d', 'Rest' = '#8f8a8a')) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5, size = 20))
```
...
for (sample in samples) {
    p <- ggplot(df[df$sample_name == sample, ], aes(x=sample_name, y = Percentage, fill = orth_closest_gene)) +
...
n_cols <- min(2, length(plots)) # Ensure not more than 4 columns
