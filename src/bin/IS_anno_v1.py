...
with open(events_file, 'r') as events_file, open(output_file, 'w', newline='') as outfile:
    ...
    for l in csv_reader:
        ...
        if id in events_dict:
            ...
            row = [sample, vector] + l[0:4] + [vector_align_clip_pos] + l[4:6] + [host_tss_info, host_tss_dist, orth_tss_info, orth_tss_dist, host_inGene, orth_inGene, enh_ingene, cpg_ingene, rmsk_anno, str(IS_reads), sonication_sites]
            writer.writerow(row)
